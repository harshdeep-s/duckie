...
class TargetFollower:
    def _init_(self):
        ...
        self.cmd_vel_pub = rospy.Publisher('/myduckie/car_cmd_switch_node/cmd', Twist2DStamped, queue_size=1)
        rospy.Subscriber('/myduckie/apriltag_detector_node/detections', AprilTagDetectionArray, self.tag_callback, queue_size=1)
        ################################################################
    ...
    def move_robot(self, detections):
        ...
        if len(detections) == 0:
            rospy.loginfo("looking for sign")
            cmd_msg = Twist2DStamped()
            cmd_msg.header.stamp = rospy.Time.now()
            cmd_msg.v = 0.0
            cmd_msg.omega = 3.0  
